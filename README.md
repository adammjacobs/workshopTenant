...
See the "Automations" section for description of each automation and its purpose. NOTE: if the automation appears to hang for longer than 30 seconds without completing, refresh the browser page.  Options that need to be selected might have been hidden by the browser and should appear if so.
